...
# 该方法将多个df数据写入到同excel的同一个sheet下面
from user_feature_merge import append_df_to_excel
...
class User_Analysis:
    ...
    def analyse_job_accuracy(self):
        ...
        """
        # Total
        job_total = self.ds1[~self.ds1['Submmit'].isin(['None'])]
        job_total['Submmit'] = pd.to_datetime(job_total['Submmit'])
        job_total = job_total[['Submmit', 'JobID']]

        Job_Total = job_total['Submmit'].groupby([
            # job_total.Submmit.dt.year.rename('year'),
            job_total.Submmit.dt.month.rename('month'),
            # job_total.Submmit.dt.week.rename('week'),
            # job_total.Submmit.dt.day.rename('day')
        ]).agg({'count'})
    ...
    def analyse_job_runtime(self):
        ...
        job_total = self.ds1[~self.ds1['Submmit'].isin(['None'])]
        job_total['Submmit'] = pd.to_datetime(job_total['Submmit'])
        job_total = job_total[['Submmit', 'CPU_Time']]
    ...
    def analyse_job_total_CPU_time(self):
        ...
        # 计算job_total的耗时数/按月算
        job_total = job_total.set_index('Submmit')
    ...
    def analyse_job_submmit_moment(self):
        """
        ...
        # Correct
        job_correct = self.ds1[~self.ds1['Done'].isin(['None'])]
        # here we still use Submmit time !!!
        job_correct['Submmit'] = pd.to_datetime(job_correct['Submmit'])
        job_correct = job_correct[['Submmit', 'Done', 'CPU_Time']]
        job_correct = job_correct[~job_correct['CPU_Time'].str.contains('unknown|None')]

        # Error
        job_error = self.ds1[~self.ds1['Completed'].isin(['None'])]
        # here we still use Submmit time !!!
        job_error['Submmit'] = pd.to_datetime(job_error['Submmit'])
        job_error = job_error[['Submmit', 'Completed', 'CPU_Time']]
        job_error = job_error[~job_error['CPU_Time'].str.contains('unknown|None')]

        # Result
        # here we still use Submmit time for total, correct and error jobs
        Moment_Total = job_total['Submmit'].groupby([
            # job_total.Submmit.dt.year.rename('year'),
            # job_total.Submmit.dt.month.rename('month'),
            # job_total.Submmit.dt.week.rename('week'),
            # job_total.Submmit.dt.day.rename('day'),
            job_total.Submmit.dt.hour.rename('hour')
        ]).agg({'count'})
        Moment_Correct = job_correct['Submmit'].groupby([
            # job_correct.Submmit.dt.year.rename('year'),
            # job_correct.Submmit.dt.month.rename('month'),
            # job_correct.Submmit.dt.week.rename('week'),
            # job_correct.Submmit.dt.day.rename('day'),
            job_correct.Submmit.dt.hour.rename('hour')
        ]).agg({'count'})
        Moment_Error = job_error['Submmit'].groupby([
            # job_error.Submmit.dt.year.rename('year'),
            # job_error.Submmit.dt.month.rename('month'),
            # job_error.Submmit.dt.week.rename('week'),
            # job_error.Submmit.dt.day.rename('day'),
            job_error.Submmit.dt.hour.rename('hour')
        ]).agg({'count'})
        ...
        # plt.show()
        plt.savefig(fname=StorePath + "\\" + SubTitle + '_job_submmit_moment.png', bbox_inches='tight')
    ...
    def analyse_job_memory_used(self):
        ...
        job_total['Submmit'] = pd.to_datetime(job_total['Submmit'])
        job_total = job_total[['Submmit', str('AVG_MEM')]]
        ...
        job_correct['Submmit'] = pd.to_datetime(job_correct['Submmit'])
        job_correct = job_correct[['Submmit', str('AVG_MEM')]]
        ...
        job_error['Submmit'] = pd.to_datetime(job_error['Submmit'])
        job_error = job_error[['Submmit', str('AVG_MEM')]]
    ...
    def prepare_for_job_submmision_habit(self):
        """
        ...
        job_total = job_total[['User','Submmit', 'Running', 'Completed', 'Done', 'CPU_Time', str('AVG_MEM')]]
    ...
    def _analyse_job_full_states_by_day(self):
        ...
        """
        # Call
        job_total = self.prepare_for_job_submmision_habit()

        # Construct
        job_total = job_total[job_total['Submmit'] > TimeNode.strftime('%Y-%m-%d %X')]
        if len(job_total) > 0:
            new_job_total = job_total[['Submmit', 'Pend', 'Error', 'Correct']]
            new_job_total['Submmit_YMD'] = new_job_total['Submmit'].map(lambda x: str(x.strftime('%Y-%m-%d')))
            ...
            Last_Days_Recorded_List = np.unique(new_job_total['Submmit_YMD'])
            for dt in Last_Days_Recorded_List:
                new_job_total_by_Day = new_job_total[new_job_total["Submmit_YMD"] == dt]
                x = new_job_total_by_Day['Submmit']
    ...
    def analyse_job_long_pending_by_day(self):
        ...
        # Construct
        new_job_total = job_total[['User', 'Submmit', 'Pend', 'Error', 'Correct']]
        # 注意，这里根据定义的 AnalysisDate 变量，拼接一下新构造出来的Submmit_HMS数据
        new_job_total['Submmit_HMS'] = new_job_total['Submmit'].map(lambda x: str(AnalysisDate) + ' ' + str(x.strftime('%H:%M:%S')))
        new_job_total['Submmit_HMS'] = pd.to_datetime(new_job_total['Submmit_HMS'])
        #print(new_job_total['Submmit_HMS'])

        job_long_pending_by_day = new_job_total[['User', 'Submmit', 'Submmit_HMS', 'Pend', 'Error', 'Correct']]
        ...
        if not os.path.exists(Current_Picture_1):
            if len(Outliers) > 0:
                x = Outliers['Submmit_HMS']
                x_start = x.min() + datetime.timedelta(hours=-1)
                x_end = x.max() + datetime.timedelta(hours=1)

                x1 = True_Outliers['Submmit_HMS']
                x2 = False_Outliers['Submmit_HMS']
        ...
        latest_job_long_pending_by_day = job_long_pending_by_day[job_long_pending_by_day['Submmit'] >= TimeNode]
        ...
        if len(latest_job_long_pending_by_day) > 0:
            ...
            if not os.path.exists(Current_Picture_2):
                if len(Latest_Outliers) > 0:
                    ...
                    x1 = Latest_True_Outliers['Submmit_HMS']
                    x2 = Latest_False_Outliers['Submmit_HMS']
    ...
    def analyse_job_submmition_counts_by_day(self):
        """
        分析用户每天的提交作业数随日期的变化
        ...
        new_job_total = job_total[['User', 'Submmit', 'Pend', 'Error', 'Correct']]
        new_job_total['Submmit_YMD'] = new_job_total['Submmit'].map(lambda x: str(x.strftime('%Y-%m-%d')))
        new_job_total['Submmit_YMD'] = pd.to_datetime(new_job_total['Submmit_YMD'])
        ...
        job_count_by_day = new_job_total[['User', 'Submmit_YMD', 'Pend_Label', 'Error_Label', 'Correct_Label']]
        Job_Count = job_count_by_day.groupby(by='Submmit_YMD').sum()

        ### 获取并返回近期的：日期统计值 和日均作业提交数，传给最后的 latest_feature 方法
        global Days_Recorded, MEAN_Submmition_Count_Recorded, MEAN_Submmition_Count
        latest_job_count_by_day = job_count_by_day[job_count_by_day['Submmit_YMD'] >= TimeNode]
        if len(latest_job_count_by_day) > 0:
            Latest_Job_Count = latest_job_count_by_day.groupby(by='Submmit_YMD').sum()
            ...
            Submmition_Count = Latest_Job_Count.apply(lambda x: x['Error_Label'] + x['Correct_Label'], axis=1)
            MEAN_Submmition_Count_Recorded = format(Submmition_Count.sum() / Days_Recorded, '.2f')
            MEAN_Submmition_Count = format(Submmition_Count.sum() / DateRange, '.2f')

            Current_Picture_1 = os.path.join(StorePath, SubTitle + '_job_submmition_counts_of_last_' + str(DateRange) + '_days.png')
            if os.path.exists(Current_Picture_1):  # I am joking here!
                os.remove(Current_Picture_1)
            if not os.path.exists(Current_Picture_1):
                ...
                ax.xaxis.set_major_formatter(dates.DateFormatter('\n%Y-%m-%d'))
                # plt.show()
                plt.savefig(fname=StorePath + "\\" + SubTitle + '_job_submmition_counts_of_last_' + str(DateRange) + '_days.png',
                            bbox_inches='tight')
                print("完成" + self.User + "用户最近" + str(DateRange) + "天的提交作业数随日期的变化分析" + "\n")
            else:
                print('我是' + self.User + '的analyse_job_submmition_counts_by_day()，Picture_1 结果文件已存在' + '\n' +
                      '可能只是被analyse_job_latest_features()调用，并没重复画图' + "\n")

        elif len(latest_job_count_by_day) == 0:
            Days_Recorded, MEAN_Submmition_Count_Recorded, MEAN_Submmition_Count = 'None', 'None', 'None'

        #Draw 这里需要两张表，一张是用户全年的作业提交表，一张是从TimeNode之后的作业提交表，如果为空，就不画
        Current_Picture_2 = os.path.join(StorePath,  SubTitle + '_job_submmition_counts_by_day.png')
        if os.path.exists(Current_Picture_2): # I am joking here!
            os.remove(Current_Picture_2)
        if not os.path.exists(Current_Picture_2):
            x = Job_Count.index
            ...
            ax.xaxis.set_major_formatter(dates.DateFormatter('\n%Y-%m-%d'))
            # plt.show()
            plt.savefig(fname=StorePath + "\\" + SubTitle + '_job_submmition_counts_by_day.png', bbox_inches='tight')
            print("完成" + self.User + "用户每天的提交作业数随日期的变化分析" + "\n")
        else:
            print('我是' + self.User + '的analyse_job_submmition_counts_by_day()，Picture_2 结果文件已存在' + '\n' +
            '可能只是被analyse_job_latest_features()调用，并没重复画图' + "\n")
    ...
    def analyse_job_latest_features(self):
        ...
        # Call
        job_total = self.prepare_for_job_submmision_habit()
        self.analyse_job_submmition_counts_by_day()
        ...
        print("最近--有提交记录的--", Days_Recorded, "天的日均作业提交数为：", MEAN_Submmition_Count_Recorded)
        print("最近", DateRange, "天的日均作业提交数为：", MEAN_Submmition_Count)
        print("------------------------------------------------------------", '\n')
        ...
        user_feature = {'User': self.User,
                        ...
                        'MEAN_Pend': MEAN_Pend,
                        'MEAN_Submmition_Count_Recorded': MEAN_Submmition_Count_Recorded,
                        'MEAN_Submmition_Count': MEAN_Submmition_Count,
                        }
        #Save
        ...
        ws.append(list(user_feature.keys()))
        ws.append(list(user_feature.values()))
        wb.save(Feature)
...
def main():
    ...
    # ua.analyse_job_submmit_moment()
    # ua.analyse_job_pend_time()
    # ua.analyse_job_memory_used()
    #
    # ua.prepare_for_job_submmision_habit()
    #
    # 此方法慎用，会比较耗时，可针对具体用户使用，使用前手动更新其方法名
    # ua.analyse_job_full_states_by_day()
    ua.analyse_job_long_pending_by_day()
    ua.analyse_job_submmition_counts_by_day()
    # ua.analyse_job_latest_features()  #返回近期的作业统计特征
    # ua.run_all_job_func()
...
if __name__ == '__main__':
    ...
    #设定时间跨度，计算原始天
    DateRange = 30
    AnalysisDate = datetime.date(2019, 12, 12)
    # AnalysisDate = datetime.date.today()
    ...
    # Queue Information Excels
    Queue_Job_Count_Excel = FolderName + '\\' + Title + '_last_' + str(DateRange) + '_days_job_count.xlsx'
    Queue_Job_Long_Pending_Excel = FolderName + '\\' + Title + '_last_' + str(DateRange) + '_days_long_pending_jobs.xlsx'
    Queue_Job_Information = FolderName + '\\' + Title + '_Queue_Info_of_last_' + str(DateRange) + '_days_from_' + AnalysisDate.strftime('%Y-%m-%d')

    # 如果存在已经解析过的 excel 表，则将其备份到队列文件夹里
    if not os.path.exists(Queue_Job_Information):
        os.mkdir(Queue_Job_Information)
    if os.path.exists(Queue_Job_Count_Excel):
        shutil.move(Queue_Job_Count_Excel, Queue_Job_Information + '\\' + "Stored_at_" + Now.strftime('%Y-%m-%d_%H.%M.%S') + '_' + os.path.basename(Queue_Job_Count_Excel))
    if os.path.exists(Queue_Job_Long_Pending_Excel):
        shutil.move(Queue_Job_Long_Pending_Excel, Queue_Job_Information + '\\'  + "Stored_at_" + Now.strftime('%Y-%m-%d_%H.%M.%S') + '_' + os.path.basename(Queue_Job_Count_Excel))
    ...
    for sub in SubFolderList:
        ...
        # SubTitle = filename
        SubTitle = sub
        Feature = os.path.join(StorePath, SubTitle + '_User_Feature.xlsx')
